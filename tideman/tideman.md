...
* Execute `unzip tideman.zip` to uncompress that file.
* Execute `rm tideman.zip` followed by `yes` or `y` to delete that ZIP file.
* Execute `ls`. You should see a directory called `tideman`, which was inside of that ZIP file.
* Execute `cd tideman` to change into that directory.
